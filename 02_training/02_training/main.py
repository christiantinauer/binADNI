...
for training_config in [
  # lr			rf		image																							mask                        									bet     	binarize		rg			shape								in feat		dtd unflatten			params destination						initial weights
	(1e-3,		.3,		'T1__bias_corrected_@_MNI152_1mm_nlin.nii.gz',		'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz',		False,   	True,				True,		(182, 218, 182),		5120,			(8, 8, 10, 8),		'T1_@MNI_nlin_bet_bin_RG',		'../01_initial_weights/{0:1d}_initial_weights_MNI.pth'),
	(1e-3,		.3,		'T1__bias_corrected_@_MNI152_1mm_nlin.nii.gz',		'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz',		False,   	False,			True,		(182, 218, 182),		5120,			(8, 8, 10, 8),		'T1_@MNI_nlin_bet_RG',				'../01_initial_weights/{0:1d}_initial_weights_MNI.pth'),
	...
	(lr, rf, image_name, mask_name, use_bet, use_binarize, use_rg, input_shape, first_linear_in_features, dtd_unflattened_dim, params_destination, initial_weights) = training_config
