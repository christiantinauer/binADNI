...
for training_config in [
  ...
  # plt.clf()
  p_to.fill_between(r, m[:, 0] - std[:, 0], m[:, 0] + std[:, 0], facecolor='blue', alpha=0.5)
  p_to.plot(r, m[:, 0], color='blue', label='training')
  p_to.fill_between(r, m[:, 1] - std[:, 1], m[:, 1] + std[:, 1], facecolor='orange', alpha=0.5)
  p_to.plot(r, m[:, 1], color='orange', label='validation')
 
  p_to.set_ylim([-0.25, 3.25])#

  p_to.set_title(model_name)
  p_to.set_xlabel('Epoch')
  p_to.set_ylabel('Loss')
...
plt.tight_layout()
plt.savefig('loss_curves.png', dpi=250)
