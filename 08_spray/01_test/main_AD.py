...
for model_heatmaps in [
  ...
  for heatmap in heatmaps: #[:n]:
    ...
    # if image_name not in AD_bootstrap[bootstrap_index]['test']:
    #   continue
...
plt.savefig("test_AD.png")

plt.clf()

plt.plot(eigvals_sym[:10], 'x')
plt.savefig("test_10_AD.png")
...
# df = df.append(tmp)
plt.scatter(X_embedded[:, 0], X_embedded[:, 1], lw=0, s=3000, alpha=0.1, color=colors[:])
...
plt.scatter(X_embedded[:, 0], X_embedded[:, 1], marker='o', color=colors[:], facecolor='none')
...
if not os.path.exists(os.path.join(target_base_path, 'bet_vs_bin_bet_clustering_AD')):
  os.makedirs(os.path.join(target_base_path, 'bet_vs_bin_bet_clustering_AD'))
...
bin_bet_paths = heatmap_paths[:]
bin_bet_labels = clustering.labels_[:]
...
for index, cluster in enumerate(clusters):
  ...
  params.append(os.path.join(target_base_path, 'bet_vs_bin_bet_clustering_AD', f'bin_bet_cluster_{index:02d}_sum.nii.gz'))
  # print(' '.join(params))
  # print(index)
  # print(cluster)
  subprocess.call(params)
