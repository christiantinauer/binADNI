...
for group_name, group in [
  ( 'A',
    [
      # inputs
      # aligned
      (0,   'A1 - Inputs',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', False, True)]),
      (2,   'A2 - Inputs',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', True, True)]),
      # heatmaps
      # aligned
      (1,   'A1 - Heatmaps',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
      (3,   'A2 - Heatmaps',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
   ]),
  ( 'B',
    [
      # inputs
      # bin1375
      (0,   'B1 - Inputs',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin__normalized__bin01375.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', False, False)]),
      (2,   'B2 - Inputs',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin__normalized__bin01375.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', True, False)]),
      # heatmaps
      # bin1375
      (1,   'B1 - Heatmaps',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bin01375/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
      (3,   'B2 - Heatmaps',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet_bin01375/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
   ]),
  ( 'C',
    [
      # inputs
      # bin275
      (0,   'C1 - Inputs',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin__normalized__bin0275.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', False, False)]),
      (2,   'C2 - Inputs',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin__normalized__bin0275.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', True, False)]),
      # heatmaps
      # bin275
      (1,   'C1 - Heatmaps',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bin0275/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
      (3,   'C2 - Heatmaps',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet_bin0275/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
   ],
  ),
  ( 'D',
    [
      # inputs
      # bin4125
      (0,   'D1 - Inputs',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin__normalized__bin04125.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', False, False)]),
      (2,   'D2 - Inputs',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin__normalized__bin04125.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', True, False)]),
      # heatmaps
      # bin4125
      (1,   'D1 - Heatmaps',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bin04125/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
      (3,   'D2 - Heatmaps',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet_bin04125/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
   ],
  ),
]:
  plt.clf()
  ...
  for model_index, (order_index, clustering_name, inputs) in enumerate(group):
    print(clustering_name)

    # row figure
    subfig = subfigs[order_index]
    subfig.suptitle(clustering_name)
    axes = subfig.subplots(nrows=1, ncols=3)

    # make output path
    clustering_name = os.path.join('appendix', f'scale_{'full' if scale is None else str(scale)}', clustering_name)
    ...
    # NC plot
    NC_right_scatter, = ax.plot(X_embedded[np.where(np.logical_and(NCs_gt, NC_pr)), 0][0], X_embedded[np.where(np.logical_and(NCs_gt, NC_pr)), 1][0], marker='x', color='green', label='TN' if order_index != 0 and order_index != 2 else 'NC', linestyle='None')
    legend_handles.append(NC_right_scatter)

    if order_index != 0 and order_index != 2:
    ...
    # AD plot
    AD_right_scatter, = ax.plot(X_embedded[np.where(np.logical_and(ADs_gt, AD_pr)), 0][0], X_embedded[np.where(np.logical_and(ADs_gt, AD_pr)), 1][0], marker='x', color='red', label='TP' if order_index != 0 and order_index != 2 else 'AD', linestyle='None')
    ...
    # savepoint
    plt.savefig(f'appendix_figure__{group_name}__all_scale_0.4.png')
