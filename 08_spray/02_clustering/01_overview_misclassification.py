...
for target_disease_group in [0, 1]:
  for scale in [
    None,
    .1,
    .4,
    ...
    subfigs = fig.subfigures(nrows=16, ncols=1)

    for model_index, (order_index, clustering_name, inputs) in enumerate([
      # inputs
      # aligned
      ...
      # (2,     'inputs_bet',                [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', True, True)]),
      # # bin1375
      ...
      # (10,    'inputs_bet_bin0275',        [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin__normalized__bin0275.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', True, False)]),
      # # bin4125
      # (12,    'inputs_full_bin04125',      [(data_base_path, 'T1__bias_corrected_@_MNI152_1mm_nlin__normalized__bin04125.nii.gz', 'T1__brain_mask_@_MNI152_1mm_nlin.nii.gz', False, False)]),
      ...
      (3,     'heatmaps_bet',              [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
      # bin1375
      ...
      (11,    'heatmaps_bet_bin0275',      [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet_bin0275/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
      # bin4125
      (13,    'heatmaps_full_bin04125',    [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bin04125/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
      ...
      # make output path
      clustering_name = os.path.join('results', f'misclass__{target_disease_group}__result__scale_{'full' if scale is None else str(scale)}', clustering_name)
      ...
      for inputs_base_path, file_name_end, mask_name, use_bet, use_normalize in inputs:
        ...
        for input in inputs_base_path_list:
          ...
          if file_name_end.startswith('__'):
            ...
            current_classification_group_index = 1 if AD_prob > .5 else 0
          else:
            subject_name = input
            ...
            current_classification_group_index = 1 if subject_name in AD_bootstrap[bootstrap_index]['test'] else 0
          ...
          current_disease_group_index = 1 if subject_name in AD_bootstrap[bootstrap_index]['test'] else 0
          if current_disease_group_index == target_disease_group and current_disease_group_index != current_classification_group_index:
            X.append(image_data.flatten())
            classification_group_index.append(current_classification_group_index)
            disease_group_index.append(current_disease_group_index)
      ...
      # NC plot
      NC_right_scatter, = ax.plot(X_embedded[np.where(np.logical_and(NCs_gt, NC_pr)), 0][0], X_embedded[np.where(np.logical_and(NCs_gt, NC_pr)), 1][0], marker='x', color='green', label='NC (TN)', linestyle='None')
      legend_handles.append(NC_right_scatter)

      NC_wrong_scatter, = ax.plot(X_embedded[np.where(np.logical_and(ADs_gt, NC_pr)), 0][0], X_embedded[np.where(np.logical_and(ADs_gt, NC_pr)), 1][0], marker='o', color='green', markerfacecolor='none', label='NC (FN)', linestyle='None')
      legend_handles.append(NC_wrong_scatter)
      
      # AD plot
      AD_right_scatter, = ax.plot(X_embedded[np.where(np.logical_and(ADs_gt, AD_pr)), 0][0], X_embedded[np.where(np.logical_and(ADs_gt, AD_pr)), 1][0], marker='x', color='red', label='AD (TP)', linestyle='None')
      legend_handles.append(AD_right_scatter)

      AD_wrong_scatter, = ax.plot(X_embedded[np.where(np.logical_and(NCs_gt, AD_pr)), 0][0], X_embedded[np.where(np.logical_and(NCs_gt, AD_pr)), 1][0], marker='o', color='red', markerfacecolor='none', label='AD (FP)', linestyle='None')
      ...
      # savepoint
      plt.savefig(f'misclass__{target_disease_group}__result__scale_{'full' if scale is None else str(scale)}.png')
