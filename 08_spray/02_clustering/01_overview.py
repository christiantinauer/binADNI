...
for scale in [
  ...
  for model_index, (order_index, clustering_name, inputs) in enumerate([
    ...
    # make output path
    clustering_name = os.path.join('results', f'scale_{'full' if scale is None else str(scale)}', clustering_name)
    ...
    # savepoint
    plt.savefig(f'result__scale_{'full' if scale is None else str(scale)}.png')
