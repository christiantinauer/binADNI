...
for scale in [
  ...
  for ax, col in zip(axes[0], ['A2', 'C2']):
    ax.set_title(col)

  for ax, row in zip(axes[:, 0], ['False Negative', 'False Positive']):
    ax.set_ylabel(row, size='large')

  fig.tight_layout()

  for model_index, (order_index, target_group_index, clustering_title, inputs) in enumerate([
    ((1, 0),    0,    'A2',   [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
    ((0, 0),    1,    'A2',   [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
    ((1, 1),    0,    'C2',   [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet_bin0275/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
    ((0, 1),    1,    'C2',   [(os.path.join(heatmaps_base_path, 'T1_@MNI_nlin_bet_bin0275/bootstrap_index-04__initial_weights_index-01'), '__downsampled.nii.gz', None, False, False)]),
  ]):  
    clustering_name = clustering_title + '_heatmaps_misclass_' + str(target_group_index)
    print(clustering_name)

    # make output path
    clustering_name = os.path.join(f'binADNI_misclass_group_means', f'scale_{'full' if scale is None else str(scale)}', clustering_name)
    ...
    for inputs_base_path, file_name_end, mask_name, use_bet, use_normalize in inputs:
      ...
      for input in inputs_base_path_list:
        ...
        if current_disease_group_index == target_group_index and current_disease_group_index != current_classification_group_index:
    ...
    # savepoint
    plt.savefig(f'binADNI_supplemental_figure_1__scale_{'full' if scale is None else str(scale)}.png')
